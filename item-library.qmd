...
crafting.df <- tibble(
  ...
  mutate(craft.profession = case_when(between(internal.index, 0, 59) ~ "Weaponsmith",
         ...
         vendor = paste0(craft.profession, ", Level ", craft.level))
...
item.cleaned.df |>
  ...
  select(basic.block, tooltip, vendor, present.div) |>
  mutate(vendor = paste0("<div><div>", if_else(!is.na(vendor), vendor, ""), "</div>", if_else(!is.na(present.div), present.div, ""), "</div>") ,
         present.div = NULL) |>
  datatable(
    rownames = FALSE, 
    escape = FALSE, 

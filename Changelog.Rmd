...
- If you load a hero from a file other than `_lastsave.txt`, `-s` and `-save` will always save the file to the same file name, unless specified otherwise
...
- Removed the `-sc` chat command. It has been superceded by the improved `-s` command
...
- **Cleave** (Barbarian):
  ...
  + Cooldown increased to 6 seconds (from 2.25 seconds)
